...
import {
  ...
  Braces, Cpu, FileJson, Hammer, Lightbulb, Users, 
        ...
        <div className="md:w-3/5 flex flex-col h-full justify-between">
          <div>
            ...
            <ul className="list-disc pl-5 space-y-1 text-gray-600 dark:text-gray-300 mb-3">
              {project.bullets && project.bullets.map((bullet: any, bulletIndex: number) => (
                <li key={bulletIndex}>
          ...
          <div>
            <div className="flex items-center gap-4">
              ...
              {/* Try Live Demo button for EV Range Predictor */}
              {project.name.includes('EV Range Predictor') && (
                <button
                  className="px-4 py-2 bg-primary-600 dark:bg-primary-500 text-white rounded-lg hover:bg-primary-700 dark:hover:bg-primary-600 transition-colors duration-200 font-medium"
                  onClick={() => navigate('/ev-range-predictor')}
                >
                  Try Live Demo
                </button>
              )}
  ...
  const allGithubProjects = [
    {
      ...
      description: "ML web app to predict electric vehicle range with React UI and FastAPI backend.",
      date: "Jul 2025",
      bullets: [
        { text: "Built ", highlight: "multiple ML models", rest: ": Random Forest, LightGBM, XGBoost, Decision Tree, Linear Regression for accurate EV range prediction" },
        { text: "Developed a ", highlight: "FastAPI backend API", rest: " for direct integration with web frontends and other services" },
        { text: "Created a ", highlight: "React web app", rest: " for interactive EV range prediction with model selection and advanced UI" },
  ...
  return (
    <Router>
      <Routes>
                  ...
                  <div className="flex flex-col gap-8 max-w-5xl mx-auto">
                    ...
                    {showAllProjects && dataScienceProjects
                      .filter(project =>
                        ...
                        project.name.includes("Cardiovascular"))
                      .map((project, index) => (
                        <ProjectCard key={"extra-" + index} project={project} />
                    ))}

                    {/* Additional Project - Tableau Dashboard - Only show when all projects are visible */}
                    {showAllProjects && (
                      <ProjectCard project={{
                        name: "Multi-Region Sales Data Dashboard",
                        description: "Interactive data visualization dashboard for regional sales analysis",
                        date: "June 2024 – Aug 2024",
                        bullets: [
                          {
                            text: "Developed an ",
                            highlight: "interactive Tableau dashboard",
                            rest: " visualizing economic data across multiple regions"
                          }
                        ],
                        technologies: ["Tableau", "Data Visualization"],
                        repoUrl: "#", // Add actual URL if available
                        icon: <BarChart size={18} className="text-primary-600 dark:text-primary-400" />,
                        category: "data-science",
                        imagePath: "/assets/projects/tableau-dashboard.jpg"
                      }} />
                    )}
                  </div>
                )}
        ...
        <Route path="/ev-range-predictor" element={<EVRangePredictor />} />
      </Routes>
    </Router>
  );
}
